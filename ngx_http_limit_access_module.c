...
typedef struct {
    ...
    ngx_queue_t                   queue;
} ngx_http_limit_access_shctx_t;


typedef struct {
    ngx_http_limit_access_shctx_t  *sh;
...
ngx_http_limit_access_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "limit_access: %i %ui.%03ui", rc, excess / 1000, excess % 1000);
    ...
    if (rc == NGX_AGAIN) {
        ...
        r->write_event_handler = ngx_http_limit_access_delay;
    ...
    n = offsetof(ngx_rbtree_node_t, color)
        + offsetof(ngx_http_limit_access_node_t, data)
        + len;

    node = ngx_slab_alloc_locked(ctx->shpool, n);
    if (node == NULL) {

        ngx_http_limit_access_expire(ctx, 0);
...
static void
ngx_http_limit_access_delay(ngx_http_request_t *r)
{
    ngx_event_t  *wev;
...
ngx_http_limit_access_init_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    ctx->sh = ngx_slab_alloc(ctx->shpool, sizeof(ngx_http_limit_access_shctx_t));
    if (ctx->sh == NULL) {
        return NGX_ERROR;
    }
    ...
    ngx_rbtree_init(&ctx->sh->rbtree, &ctx->sh->sentinel,
                    ngx_http_limit_access_rbtree_insert_value);
...
ngx_http_limit_access_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ctx == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "no variable is defined for limit_access_zone \"%V\"",
                           &cmd->name);
        return NGX_CONF_ERROR;
    }
